...
apply_demand_response <- function(base_demand_orders,
                                  scenario_row,
                                  is_peak = FALSE) {
  base_price <- scenario_row$customer_price_per_trip[[1]]
  peak_rider_surcharge <- scenario_row$peak_rider_surcharge[[1]]
  elasticity <- scenario_row$demand_price_elasticity[[1]]
  
  effective_customer_price <- ifelse(
    is_peak,
    base_price + peak_rider_surcharge,
    base_price
  )
  
  pct_price_change <- safe_divide(effective_customer_price - base_price, base_price)
...
match_market_hour <- function(demand_row,
  ...
  effective_customer_price <- ifelse(
    is_peak,
    customer_price_per_trip + peak_rider_surcharge,
  ...
  supply_df <- supply_df %>%
    dplyr::mutate(
      ...
      avg_hourly_earnings =
        (avg_trips_per_active_worker * worker_pay_per_trip * effective_surge) +
        effective_bonus
    )
  ...
  platform_revenue <- fulfilled_orders * effective_customer_price
  worker_cost <- (fulfilled_orders * worker_pay_per_trip * effective_surge) + total_bonus_cost
  platform_margin <- platform_revenue - worker_cost
  
  hourly_results <- tibble::tibble(
